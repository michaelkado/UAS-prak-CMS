...
class Komentar extends Ci_controller
{
	...
	public function add()
	{
		if ($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			if (!$this->upload->do_upload('komen_image'))
			{
			...
			else
			{
				...
				$img_komen = $data_upload['file_name'];

				$aksi = $this->komentar_model->add($img_komen);
				if ($aksi) 
				{
			...
			$data['judul'] = "tambah data komentar";
			//load template
			$tmp['konten'] = $this->load->view("admin/komentar/add",$data,TRUE);
			$this->load->view('admin/template',$tmp);
		}
	...
	public function edit($id=0)
	{
		if ($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			{//jika terdapat data yang akan diedit
				$file_lama = $data_komentar['KOMEN_IMAGE'];
				...
				if ($file_lama != "") 
				{
					...
					else
					{
						...
						$data_upload = $this->upload->data();
						$img_komen = $data_upload['file_name'];
						$aksi = $this->komentar_model->update_by_admin($id,$img_komen);
						if ($aksi) 
						{
		...
		{
			$data_komentar = $this->komentar_model->get_detail_by_id($id);
			$data['judul'] = "edit data komentar  ".$data_komentar['FID_PELANGGAN'];
